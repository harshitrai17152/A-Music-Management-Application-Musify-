...
import javafx.stage.Stage;
 public class filmmovie extends Application {
    ...
    public void start(Stage primaryStage) throws SQLException {
		...
		String q="select * from FilmMakers";
		ResultSet rs=sta.executeQuery(q);    
		while(rs.next())  
        {  
        	if(rs.getInt(1)==filmID)
        		lv.getItems().add(rs.getString(2)+"   --   "+rs.getString(5)+"   --   "+rs.getString(6)+"   --   "+rs.getString(7));
        } 
		 
		EventHandler<ActionEvent> event=new EventHandler<ActionEvent>() { 
            public void handle(ActionEvent e) 
            { 
            	new filmpage(filmID).start(primaryStage);
            } 
        }; 
